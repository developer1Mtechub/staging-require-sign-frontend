...
const CompletedDocViewReceiver = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const [loading,setLoading]=useState(true)
  const logo = useSelector(selectLogo);
  // const loading = useSelector(selectLoading);
const [company_logo,setComp_Logo]=useState(logo)
  ...
  // Fetch File
  const fetchFileData = async (fileId,receipt_email) => {
    // get Images from db
    const postData = {
      file_id: fileId,
...
setComp_Logo(apiData?.sender?.logo)
setLoading(false)
    ...
    // if (status === "succeeded") {
      const fetchDataBasedOnUser = async () => {
        try {
          ...
          await Promise.all([
            fetchData(file_id),
            fetchFileData(file_id,receipt_email),
...
export default CompletedDocViewReceiver;
