...
import {TabContent, TabPane, Nav, NavItem, NavLink, Col, Row, Card, Label, Button} from 'reactstrap';
...
const SignatureModalContent = ({
  ...
  useEffect(() => {
    console.log("initialsBox",initialsBox)
    if (profile) {
        ...
        <Row>
          <Col sm="12">
            <div
              ...
              {signatureChooseImage ? (
                <h1 className="mb-1 fw-bold">{t("Add Image")}</h1>
              ) : (
                <>
                  {initialsBox ? (
          ...
          {PrevSignatureArrayImageInitial.length} */}
            <TabContent className="py-50" activeTab={active}>
              ...
              <TabPane tabId={2}>
                <ImagePicker  user_id_user={user?.user_id}
                 user_email={user?.email}  lengthPrevsign={PrevSignatureArrayImage.length}
              ...
              <TabPane tabId={3}>
                <TypeText user_first_name={user?.first_name} 
...
<Row style={{height: '250px', overflowY: 'auto',display:"flex"}}>
      ...
      />
       <ModalConfirmationAlert
        ...
        alertStatusDelete={'delete'}
        text={t("Are you sure you want to delete this Initials?")}
      />
      {/* Tabs  */}
